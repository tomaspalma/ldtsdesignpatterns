...
public class StringTransformerGroupTest {
    ...
    public void transformerGroup() {
        var drink = new StringDrink("AbCd-aBcD");

        var si = new StringInverter();
        var cc = new StringCaseChanger();

        List<StringTransformer> transformers = new ArrayList<>();
        Assertions.assertTrue(transformers.add(si));
        Assertions.assertTrue(transformers.add(cc));

        var tg = new StringTransformerGroup(transformers);
        tg.execute(drink);
